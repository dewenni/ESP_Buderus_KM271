...
#define PAYLOAD_LEN 128
  ...
  // restart ESP command
  if (strcmp(topic, addTopic(mqttCmd.RESTART[config.mqtt.lang])) == 0) {
  ...
  // send sim data
  else if (strcmp(topic, addTopic(mqttCmd.SIMDATA[config.mqtt.lang])) == 0) {
  ...
  // Service command
  else if (strcmp(topic, addTopic(mqttCmd.SERVICE[config.mqtt.lang])) == 0) {
  ...
  else if (strcmp(topic, addTopic(mqttCmd.DEBUG[config.mqtt.lang])) == 0) {
    if (intVal == 1) {
  ...
  // set debug filter
  else if (strcmp(topic, addTopic(mqttCmd.SET_DEBUG_FLT[config.mqtt.lang])) == 0) {
    char errMsg[256];
  ...
  // get debug filter
  else if (strcmp(topic, addTopic(mqttCmd.GET_DEBUG_FLT[config.mqtt.lang])) == 0) {
    km271Msg(KM_TYP_MESSAGE, config.debug.filter, "");
  }
  // set date and time
  else if (strcmp(topic, addTopic(mqttCmd.DATETIME[config.mqtt.lang])) == 0) {
    km271SetDateTimeNTP();
  }
  // set oilmeter
  else if (strcmp(topic, addTopic(mqttCmd.OILCNT[config.mqtt.lang])) == 0) {
    cmdSetOilmeter(intVal);
  }

  // HK1 Betriebsart
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_OPMODE[config.mqtt.lang])) == 0) {
  ...
  // HK2 Betriebsart
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_OPMODE[config.mqtt.lang])) == 0) {
    if (isNumber(payloadCopy)) {
  ...
  // HK1 Programm
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_PROGRAM[config.mqtt.lang])) == 0) {
    if (isNumber(payloadCopy)) {
      km271sendCmd(KM271_SENDCMD_HC1_PROGRAMM, intVal);
    } else {
      int targetIndex = -1;
      for (int i = 0; i < 9; i++) {
        if (strcmp(cfgArrays.HC_PROGRAM[config.mqtt.lang][i], payloadCopy) == 0) {
          targetIndex = i;
          break;
        }
      }
  ...
  // HK2 Programm
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_PROGRAM[config.mqtt.lang])) == 0) {
    if (isNumber(payloadCopy)) {
  ...
  // HK1 Auslegung
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_INTERPR[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC1_DESIGN_TEMP, intVal);
  }
  // HK2 Auslegung
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_INTERPR[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC2_DESIGN_TEMP, intVal);
  }
  // HK1 Aussenhalt-Ab Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_SWITCH_OFF_THRESHOLD[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC1_SWITCH_OFF_THRESHOLD, intVal);
  }
  // HK2 Aussenhalt-Ab Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_SWITCH_OFF_THRESHOLD[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC2_SWITCH_OFF_THRESHOLD, intVal);
  }
  // HK1 Tag-Soll Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_DAY_TEMP[config.mqtt.lang])) == 0) {
    km271sendCmdFlt(KM271_SENDCMD_HC1_DAY_SETPOINT, floatVal);
  }
  // HK2 Tag-Soll Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_DAY_TEMP[config.mqtt.lang])) == 0) {
    km271sendCmdFlt(KM271_SENDCMD_HC2_DAY_SETPOINT, floatVal);
  }
  // HK1 Nacht-Soll Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_NIGHT_TEMP[config.mqtt.lang])) == 0) {
    km271sendCmdFlt(KM271_SENDCMD_HC1_NIGHT_SETPOINT, floatVal);
  }
  // HK2 Nacht-Soll Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_NIGHT_TEMP[config.mqtt.lang])) == 0) {
    km271sendCmdFlt(KM271_SENDCMD_HC2_NIGHT_SETPOINT, floatVal);
  }
  // HK1 Ferien-Soll Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_HOLIDAY_TEMP[config.mqtt.lang])) == 0) {
    km271sendCmdFlt(KM271_SENDCMD_HC1_HOLIDAY_SETPOINT, floatVal);
  }
  // HK2 Ferien-Soll Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_HOLIDAY_TEMP[config.mqtt.lang])) == 0) {
    km271sendCmdFlt(KM271_SENDCMD_HC2_HOLIDAY_SETPOINT, floatVal);
  }
  // WW Betriebsart
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.WW_OPMODE[config.mqtt.lang])) == 0) {
    if (isNumber(payloadCopy)) {
  ...
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_SUMMER_THRESHOLD[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC1_SUMMER, intVal);
  }
  // HK1 Frost-Ab Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_FROST_THRESHOLD[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC1_FROST, intVal);
  }
  // HK2 Sommer-Ab Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_SUMMER_THRESHOLD[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC2_SUMMER, intVal);
  }
  // HK2 Frost-Ab Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_FROST_THRESHOLD[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC2_FROST, intVal);
  }
  // WW-Temperatur
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.WW_TEMP[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_WW_SETPOINT, intVal);
  }
  // HK1 Ferien Tage
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_HOLIDAY_DAYS[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC1_HOLIDAYS, intVal);
  }
  // HK2 Ferien Tage
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_HOLIDAY_DAYS[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC2_HOLIDAYS, intVal);
  }
  // WW Pump Cycles
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.WW_CIRCULATION[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_WW_PUMP_CYCLES, intVal);
  }
  // HK1 Reglereingriff
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_SWITCH_ON_TEMP[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC1_SWITCH_ON_TEMP, intVal);
  }
  // HK2 Reglereingriff
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_SWITCH_ON_TEMP[config.mqtt.lang])) == 0) {
    km271sendCmd(KM271_SENDCMD_HC2_SWITCH_ON_TEMP, intVal);
  }
  // HK1 Absenkungsart
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC1_REDUCTION_MODE[config.mqtt.lang])) == 0) {
    if (isNumber(payloadCopy)) {
      km271sendCmd(KM271_SENDCMD_HC1_REDUCTION_MODE, intVal);
    } else {
      int targetIndex = -1;
      for (int i = 0; i < 4; i++) {
        if (strcmp(cfgArrays.REDUCT_MODE[config.mqtt.lang][i], payloadCopy) == 0) {
          targetIndex = i;
          break;
        }
      }
  ...
  // HK2 Absenkungsart
  else if (strcmp(topic, addCfgCmdTopic(cfgTopics.HC2_REDUCTION_MODE[config.mqtt.lang])) == 0) {
    if (isNumber(payloadCopy)) {
